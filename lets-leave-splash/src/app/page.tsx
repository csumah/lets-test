...
export default function SplashPage() {
  ...
  return <SplashScreen onLogin={goToMap} onGuest={goToMap} />
}

// ====== SplashScreen (from your Vite app, slightly adapted) ======
...
function SplashScreen({ onLogin, onGuest }: SplashScreenProps) {
  ...
  const [open, setOpen] = useState<
    'how' | 'a11y' | 'settings' | 'privacy' | 'terms' | null
  >(null)
    ...
    >
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 items-center">
            ...
            >
              <img
                ...
                className="w-[460px] max-w-[40vw] drop-shadow-xl"
              />
            </motion.div>
          </div>
        </div>
      ...
      <footer
        ...
        style={{
          ...
          boxShadow:
            '0 -1px 0 rgba(255,203,5,0.6) inset, 0 -10px 30px rgba(0,0,0,0.35) inset',
        }}
      >
...
type ModalProps = {
  ...
  children: ReactNode
  onClose: () => void
  brand: { gold: string; black: string; ink: string; white: string }
}

/* Reusable centered modal */
function Modal({ title, children, onClose, brand }: ModalProps) {
  return (
    <>
      <motion.div
