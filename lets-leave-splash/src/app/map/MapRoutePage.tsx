...
export default function MapRoutePage() {
  ...
  const [open, setOpen] = useState<'how' | 'a11y' | 'settings' | null>(null)
  ...
  const startDrag = (e: React.MouseEvent | React.TouchEvent) => {
    ...
    const onMove = (ev: MouseEvent | TouchEvent) => {
      ...
      setLeftPct(clamp(startLeft + delta, 0, 100))
    }
  ...
  const pillPosStyle = useMemo(() => {
    if (leftPct <= 6) return { left: 16, transform: 'translateY(-50%)' as const }
    if (leftPct >= 94) return { right: 16, transform: 'translateY(-50%)' as const }
    return { left: '50%', transform: 'translate(-50%, -50%)' as const }
  }, [leftPct])
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* RIGHT — directions */}
        <div
          className="relative overflow-hidden z-0 flex items-center justify-center"
      ...
      <footer
        className="flex-none w-full px-6 py-3 text-white"
      ...
      >
        <div className="flex items-center justify-between gap-3">
          ...
          <div className="text-sm opacity-60">
            © {new Date().getFullYear()} Let&apos;s Leave
          </div>
        </div>
      </footer>
              ...
              <div className="space-y-3">
                {savedRoutesList.map((r) => (
                  <button
                    ...
                    className="w-full text-left p-3 rounded-lg bg.white/10 bg-white/10 border border-white/20 hover:bg-white/20 transition"
                  >
                    {r.name}
      ...
      <AnimatePresence>
        {open === 'how' && (
          <Modal onClose={() => setOpen(null)} title="How it works" brand={brand}>
        ...
        {open === 'a11y' && (
          <Modal onClose={() => setOpen(null)} title="Accessibility" brand={brand}>
        ...
        {open === 'settings' && (
          <Modal onClose={() => setOpen(null)} title="Settings" brand={brand}>
            <div className="space-y-3">
              <button
                onClick={resetSplit}
...
type MapModalProps = {
  ...
  children: ReactNode
  onClose: () => void
  brand: { gold: string; black: string; ink: string }
}

/** Reusable modal */
function Modal({ title, children, onClose, brand }: MapModalProps) {
  return (
    <>
      <motion.div
        className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[90]"
